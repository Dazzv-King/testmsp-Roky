<template>
  <div class="container">
    <div class="testmsp">
      <div class="other__city">
        <h2>Город: {{staticCity}}</h2>
        <input type="input" class="input__city" v-model="selectedCity" placeholder="Выберите город">
      </div>
      <ul class="cities" v-if="selectedCity.length && filteredCities.length">
        <li class="city"
        v-for="(city, index) in filteredCities"
        :key="index"
        @click="getNewCityLoc(city)"
        >
          <h4>{{city.name}}</h4>
        </li>
      </ul>
...
export default {
  ...
  methods: {
    getNewCityLoc(city) {
      this.selectedCity = ''
      this.staticCity = city.name

      this.center.lat = Number(city.coords.lat)
      this.center.lng = Number(city.coords.lon)
  ...
  mounted() {
    ...
    if(city){
      ...
      this.staticCity = selectedCity.name

      this.center.lat = Number(selectedCity.coords.lat)
      this.center.lng = Number(selectedCity.coords.lon)
...
<style src='.\assets\styles.scss' lang='scss'>
